...
export default function AccountNew() {
    ...
    useEffect(() => {
      const func =async()=>{   
        ...
        if(data==="jr_acc"){
          const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_jr_acc/")
          setdata3( await response2.json())
        }
        else if(data==="jr_acc_officer"){
          const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_jr_acc_officer/")
          setdata3( await response2.json())
          
        }
        else if(data ==="acc_asst_registrar"){
          const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_acc_asst_registrar/")
          setdata3( await response2.json())
        }
        else if(data ==="acc_deputy_registrar"){
            const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_acc_deputy_registrar/")
            setdata3( await response2.json())
          }
        }
        func()
    ...
    const search_empCode=async()=>{
      set_emp_flag(1);
      if(user_t==='jr_acc'){
        const response1 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_jr_acc/"+emp)
        const data1 = await response1.json();
        set_table_data(data1)
      }
      else if(user_t ==="jr_acc_officer"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_jr_acc_officer/"+emp)
        set_table_data( await response2.json())
      }
  
      else if(user_t ==="acc_asst_registrar"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_acc_asst_registrar/"+emp)
        set_table_data( await response2.json())
      }

      else if(user_t ==="acc_deputy_registrar"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_acc_deputy_registrar/")
        set_table_data( await response2.json())
      }
