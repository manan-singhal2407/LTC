...
export default function ViewAppForm4(){
    ...
    const [acc_table_data,set_acc_table_data] = useState([]);
    const [data_est,setdata_est]=useState({
        ...
        const [data_acc,setdata_acc]=useState({
            total:0,
            advanced:0,
            passed:0,
            words:"",
        ...
        useEffect(() => {
            const func= async()=>{
                ...
                const response6 = await fetch("https://ltc-mgmt.herokuapp.com/getaccountsdata/"+reqid)
                const data6 = await response6.json()
                setdata_acc(data6);

                const response7 = await fetch("https://ltc-mgmt.herokuapp.com/getaccounts_tabledata/"+reqid)
                const data7 = await response7.json()
                console.log(data7);
                set_acc_table_data(data7);
        ...
        const approve =async()=>{
            if(user_t==="jr_acc_officer"){
            ...
            else if(user_t==="main_registrar"){
                navigate("/registrarpage/"+auth.currentUser.uid+"/new")
                const response = await fetch("https://ltc-mgmt.herokuapp.com/main_registrar_update/",{
                method:"POST",
                headers: {"Content-type":"application/json"},
                body: JSON.stringify(e)
            });
                ...
                const response = await fetch("https://ltc-mgmt.herokuapp.com/dean_update/",{
                method:"POST",
                headers: {"Content-type":"application/json"},
                body: JSON.stringify(e)
            });
        ...
        const revert =async()=>{
            if(user_t==="jr_acc_officer"){
                ...
                navigate("/registrarpage/"+auth.currentUser.uid+"/new")
                const response = await fetch("https://ltc-mgmt.herokuapp.com/main_registrar_revert/",{
                method:"POST",
                headers: {"Content-type":"application/json"},
                body: JSON.stringify(e)
            });
                ...
                navigate("/deanpage/"+auth.currentUser.uid+"/new")
                const response = await fetch("https://ltc-mgmt.herokuapp.com/dean_revert/",{
                method:"POST",
                headers: {"Content-type":"application/json"},
                body: JSON.stringify(e)
            });
    ...
    return(
        ...
        <div className={styles.container}>
            ...
            <p style={{fontWeight: "650",textDecoration: "underline",textAlign:"center",fontSize:"16px",marginBottom:"30px"}}>FOR USE BY THE ACCOUNT SECTION</p>
            <table className={styles.hod} style={{width: "100%"}}>
                <tr>
                    <th style={{width:"17%"}} className={styles.tag}>From</th>
                    <th style={{width:"16.6%"}} className={styles.tag}>To</th>
                    <th style={{width:"16.6%"}} className={styles.tag}>Mode Of Travel</th>
                    <th style={{width:"16.6%"}} className={styles.tag}>No. of Fares</th>
                    <th style={{width:"16.6%"}} className={styles.tag}>Single Fare</th>
                    <th style={{width:"150px"}} className={styles.tag}>Amount</th>
                </tr>
                {acc_table_data.map((element, index) => (
                    <tr data-index={index}>
                        <td className={styles.field}>{element.from}</td>
                        <td className={styles.field}>{element.to}</td>
                        <td className={styles.field}>{element.mode}</td>
                        <td className={styles.field}>{element.fares}</td>
                        <td className={styles.field}>{element.single_fare}</td>
                        <td className={styles.field}>{element.amount}</td>
                    </tr>
                ))}
            </table>
            <p style={{fontSize: "14px", marginTop:"25px"}}><span style={{fontWeight: "500"}}>Total Rs. </span>{data_acc.total}</p>
            <p style={{fontSize: "14px"}}><span style={{fontWeight: "500"}}>Advance admissible (90% of above) = Rs. </span>{data_acc.advanced}<span style={{fontWeight: "500", marginLeft:"15px"}}>Passed for Rs. </span>{data_acc.passed}</p>
            <p style={{fontSize: "14px"}}><span style={{fontWeight: "500"}}>(in words); Rupees </span>{data_acc.words}</p>
            <p style={{fontSize: "14px"}}><span style={{fontWeight: "500"}}>Debitable to LTC advance Dr./Mr./Mrs./Ms. </span>{data_acc.debit}</p>
            {flag!=1?
            <>
                ...
                <div className={styles.btns}>
                    ...
                    <button type="button" className={styles.forward} onClick={approve}>Forward</button>
                </div>
            </>:<></>}
        </div>
        </>
