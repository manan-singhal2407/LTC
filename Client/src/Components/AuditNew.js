  ...
  const [user_t,set_user]=useState("")
    ...
    useEffect(() => {
      const func =async()=>{   
        ...
        if(data==="asstauditofficer"){
          const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_asst_audit_officer/")
          setdata3( await response2.json())
        }
        else if(data==="audit_officer"){
          const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_audit_officer/")
          setdata3( await response2.json())
        }
        else if(data ==="senior"){
          const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_application_sr_audit_officer/")
          setdata3( await response2.json())
        }
        }
        func()
    ...
    const search_empCode=async()=>{
      ...
      if(user_t==='asstauditofficer'){
        const response1 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_asstauditofficer/"+emp)
        const data1 = await response1.json();
        set_table_data(data1)
      }
      else if(user_t ==="audit_officer"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_audit_officer/"+emp)
        set_table_data( await response2.json())
      }
  
      else if(user_t ==="senior"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_senior/"+emp)
        set_table_data( await response2.json())
      }
