...
export default function RegistrarPrevious() {
 
    const [data3,setdata3]=useState([]);
    useEffect(() => {
      const func = async ()=>{
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/getprevious_main_registrar/")
    ...
    const viewApp = async(event)=>{
      ...
      const flag1=1;
      // console.log(reqId,auth.currentUser.uid,flag1,"OKOKOKOK")
    ...
    const back_btn = () => {
      ...
      const search_empCode=async()=>{
        set_emp_flag(1);
        const response1 = await fetch("https://ltc-mgmt.herokuapp.com/get_prev_emp_applications_main_registrar/"+emp)
