...
export default function AuditPrevious() {
  ...
  useEffect(() => {
    const func = async ()=>{
      ...
      if(data ==="asstauditofficer"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/getprevious_audit/")
      setdata3( await response2.json())
      }
      else if(data ==="audit_officer"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/getprevious_audit_officer/")
        setdata3( await response2.json())
      }

      else if(data ==="senior"){
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/getprevious_sr_officer/")
        setdata3( await response2.json())
      }
  ...
  const search_empCode=async()=>{
    set_emp_flag(1);
    if(user_t==='asstauditofficer'){
      const response1 = await fetch("https://ltc-mgmt.herokuapp.com/get_prev_emp_applications_asst_audit_officer/"+emp)
      const data1 = await response1.json();
      set_table_data(data1)
    }
    else if(user_t ==="audit_officer"){
      const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_prev_emp_applications_audit_officer/"+emp)
      set_table_data( await response2.json())
    }

    else if(user_t ==="senior"){
      const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_prev_emp_applications_sr_audit_officer/"+emp)
      set_table_data( await response2.json())
    }
