...
export default function RegistrarNew() {
    ...
    useEffect(() => {
      const func =async()=>{   
        const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_registrar/")
    ...
    const search_empCode=async()=>{
      set_emp_flag(1);
      const response1 = await fetch("https://ltc-mgmt.herokuapp.com/get_new_emp_applications_main_registrar/"+emp)
