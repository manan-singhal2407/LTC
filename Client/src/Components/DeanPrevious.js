...
export default function DeanPrevious() {
    const navigate=useNavigate()
    const [data3,setdata3]=useState([]);

    useEffect(() => {
        const func =async()=>{
            const response2 = await fetch("https://ltc-mgmt.herokuapp.com/get_prev_app/")
  ...
  const viewApp = async(event)=>{
    ...
    const search_empCode=async()=>{
      set_emp_flag(1);
      const response1 = await fetch("https://ltc-mgmt.herokuapp.com/get_prev_emp_applications_dean/"+emp)
